...
{
    //===========================2D Perlin Noise==========================
    [Header("2D Perlin Noise")]
    [SerializeField] private float noiseTextureSamplingScale;
    [SerializeField] private int pixelWidth = 1024;
    [SerializeField] private int pixelHeight = 1024;
    private Texture2D _noiseTexture;
    private Color[] _pixels;
    
    //===========================3D Perlin Noise==========================

    [Header("3D Perlin Noise")]
    public int repeat;
    ...
    private void Start()
    {
        _marchingCubes = GetComponent<MarchingCubes>();
    }
    ...
    private float SampleTexture(Vector2 sampleCoordinates)
    {
        ...
        sampleCoordinates.x %= pixelWidth + 1;
        sampleCoordinates.y %= pixelHeight + 1;
    ...
    public float PerlinNoise2D(Vector2 pCoordinates)
    {
        return Mathf.PerlinNoise(pCoordinates.x, pCoordinates.y);
    }
    ...
    private float GradientVector(int hash, float x, float y, float z)
    {
        /*
    ...
    private void AssembleScalarField()
    {
        ...
        Vector3 startingPosition = transform.position - transform.localScale / 2;

        //Calculate the size of a step for all dimensions
        Vector3 volumeVector = _marchingCubes.VolumeVector;
        float vertexStepSizeX = currentLocalScale.x / (volumeVector.x - 1);
        float vertexStepSizeY = currentLocalScale.y / (volumeVector.y - 1);
        float vertexStepSizeZ = currentLocalScale.z / (volumeVector.z - 1);
        ...
        for (int x = 0; x < volumeVector.x; x++)
        {
            for (int y = 0; y < volumeVector.y; y++)
            {
                for (int z = 0; z < volumeVector.z; z++)
                {
                    ...
                    if (perlinNoiseValue < _marchingCubes.SurfaceValue) continue;
                    //Create primitive
...
public static class VectorExtensions
{
    public static Vector3 Mod (this Vector3 vector, float modulo)
    {
        ...
        //Had to do it this way since C# currently does not support extension operator overloading
